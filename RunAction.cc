...
RunAction::RunAction(): photonHits(40, 0)
{
 ...
 /* for (G4int k = 0; k < 20; k++){
  ...
  man->CreateNtupleDColumn("Particle_Momentum_GeV"); //60
  //man->CreateNtupleDColumn("angle");  
  man->CreateNtupleDColumn("Copy_num_Bar_Traversed_A"); //61
  man->CreateNtupleDColumn("Copy_num_Bar_Traversed_B"); //62
  man->CreateNtupleIColumn("HIT_particle_passed_both_layers");  // 63

  man->CreateNtupleDColumn("Total_Energy_Deposition");  //64
  man->CreateNtupleDColumn("Total_dEdx");  //65
  //man->CreateNtupleIColumn("Total_Photons_Detected");  
  man->CreateNtupleDColumn("Total_Photons_Generated");  //66

  //man->CreateNtupleSColumn("PARTICLE_NAME_ON_A"); 
  //man->CreateNtupleSColumn("PARTICLE_NAME_ON_B"); 
